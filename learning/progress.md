...
**Next up:** **T11 ServiceMonitor** (+ the infra-controller cleanup sweep). **NEW deferred chapter — "PromQL & metric joins"** (`group_left`, owner-chain, the cАdvisor×KSM rollup — T10 Q4 deferred here by user request); slot it around the query-path/Grafana topics. (cAdvisor inserted as **T10** → ServiceMonitor=T11, PodMonitor=T12, Prometheus Operator=T13, rest +1.)
