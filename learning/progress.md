...
**Current focus:** Phase 1, deep-dive depth inline. **T7 Exporters, T8 node-exporter, T9 KSM all MASTERED 2026-06-14**, each with a live **cleanup capstone** (per-job cardinality optimization on the meta-monitoring stack — tracked in `_meta_monitoring/OPTIMIZATION.md`). **T10 cAdvisor (+kubelet) TAUGHT 2026-06-14 (`eod/Topic10.md`); cleanup DONE** (container_ firehose −73%, cluster ingest −9.3k); **quiz PENDING**. Cumulative sweep so far: cluster `samples_ingested` **52,775 → 37,315 (~29%)**.
**Next up:** **T10 cAdvisor quiz** (4 Qs in `eod/Topic10.md`), then **T11 ServiceMonitor**. (cAdvisor inserted as **T10** → ServiceMonitor=T11, PodMonitor=T12, Prometheus Operator=T13, the rest shift +1.)
...
| 10 | **cAdvisor (+kubelet)** | ✅ | – (quiz pending) | — | **TAUGHT 2026-06-14; cleanup −73% (cadvisor 6357→1740/target).** Embedded in kubelet; cgroups→`container_*`; scraped `:10250/metrics/cadvisor` by the daemonset; #1 firehose + churn; join to KSM for node/workload rollup; **only lever = `metric_relabel_configs` keep-list** (no native knob). **Quiz: 4 Qs + answer key in eod/Topic10.md.** |
| 11 | ServiceMonitor | ⬜ | – | P3·P5 | (was T10; +1 after cAdvisor insert) |
